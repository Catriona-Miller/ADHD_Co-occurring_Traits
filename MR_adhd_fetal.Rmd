...
# Exposure data
# From here until after clumping has already been done on this data for autism so will not be run again
```{r}
...
# Now remove SNPs
```{r}
#fetal_exposure_clumped_rem = subset(fetal_exposure_clumped,!(SNP %in% removal_snps$rsidt))
```


# Save exposure data
```{r}
#write.csv(fetal_exposure, file = 'fetal_exposure.csv')
```
...
ggsave("MR_results_ADHD_adult.pdf", width = 5, height =2, useDingbats=FALSE)
```